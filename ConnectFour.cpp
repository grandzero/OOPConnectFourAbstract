...
ConnectFour::ConnectFour(int newHeight,int newWidth) : height(newHeight),width(newWidth) {
    if(isSizeLegal(newHeight,newWidth)){
        cerr<<"Wrong size ! Size must be bigger than 0 !"<<endl;
        ConnectFour();
    }
    setGameCells();
}
...
void ConnectFour::read(){
    ...
    /*Before LOAD clears old board*/
    for(int i=0;i<height;++i)delete[] gameBoard[i];delete[] gameBoard;
    int j=0;
...
/*****************************************************************/
	...
	if(cin.eof())
		exit(0);
	cout<<"ConnectFour Play worked"<<endl;
        if(input[0]=='S'&&input[1]=='A'&&input[2]=='V'&&input[3]=='E'){
